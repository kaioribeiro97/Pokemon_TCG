...
// 1. Remove sufixos de raridade e variações de forma/clima
// Adicionados: Lavagem, Corte, Calor, Ventilador
let cleaned = fullName.replace(/\s(ex|GX|VMAX|VSTAR|V|Tera|TAG\sTEAM|EX|Prime|LEGEND|ex\sTera|LV\.X|Lavagem|Corte|Calor|Ventilador)\b/gi, '');

// 2. Remove posse/treinador (Mantida igual, ela já limpa o "do..." se vier no fim)
cleaned = cleaned.replace(/\s(da|do|de)\s.+$/i, '');

// 3. Remove prefixos de Mega Evolução e Treinadores específicos no início
// Adicionados: Brock's e Onix do (com o ^ para garantir que estão no começo)
cleaned = cleaned.replace(/^(Mega|M|Brock's|Onix do)\s/i, '');

// 4. Garante que não fiquem espaços duplos perdidos
...
async function loadData() {
    ...
    function attachCollectionListener(colId) {
      ...
      state.unsubscribeMap[colId] = onSnapshot(docRef, (snap) => {
        ...
        if (state.collections[colId]) {
          state.collections[colId].rawData = data;
  ...
  filtered.forEach(card => {
    ...
    // --- 1. LÓGICA DE TAGS (NOVO) ---
    // Pega as tags que já vêm no JSON (ex: cartas especiais fixas)
    const jsonTags = card.tags || []; 
    
    // Pega as tags que VOCÊ salvou (ex: checkbox "Foil" no modal)
    let userTags = [];
    // Verifica se temos os dados brutos (rawData) carregados na Parte A
    if (isOwned && col.rawData && col.rawData[cardId] && typeof col.rawData[cardId] === 'object') {
        userTags = col.rawData[cardId].tags || [];
    }

    // Junta tudo e remove duplicatas (Set)
    const allTags = [...new Set([...jsonTags, ...userTags])];

    // Gera o HTML das etiquetas
    let tagsHTML = "";
    if (allTags.length > 0) {
        tagsHTML = `<div class="flex flex-wrap gap-1 mt-1 mb-0.5 justify-center w-full px-1">
          ${allTags.map(tag => {
            let styleClass = "bg-slate-700 text-slate-300 border-slate-600"; // Cor Padrão
            
            // Personalização de Cores
            const t = tag.toLowerCase();
            if (t.includes("foil") || t.includes("holografica")) styleClass = "bg-amber-500/20 text-amber-300 border-amber-500/50 shadow-[0_0_6px_rgba(245,158,11,0.2)]";
            else if (t.includes("japonesa") || t === "jp") styleClass = "bg-rose-900/40 text-rose-300 border-rose-700";
            else if (t.includes("ingles") || t === "en") styleClass = "bg-indigo-900/40 text-indigo-300 border-indigo-700";
            else if (t.includes("rara")) styleClass = "bg-purple-900/40 text-purple-300 border-purple-700";
            
            return `<span class="px-1.5 py-0.5 rounded text-[7px] font-bold uppercase border ${styleClass}">${tag}</span>`;
          }).join("")}
        </div>`;
    }
    ...
    container.className = `flex flex-col gap-1 items-center bg-slate-900/40 p-2 rounded-2xl border border-slate-800 hover:border-slate-700 transition`;
    ...
    if (isOwned) {
        // --- LÓGICA MATEMÁTICA DO CODEPEN (Mantida igual) ---
